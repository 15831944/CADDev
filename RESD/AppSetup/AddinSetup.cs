...
namespace RESD.AppSetup
{
    ...
    internal class AddinSetup : IExtensionApplication
    {
        void IExtensionApplication.Initialize()
        {
            ...
            // 
            Application.DocumentManager.MdiActiveDocument.Editor.WriteMessage($"{TabId_RESD}程序加载成功\n");
        }
        ...
        //添加自定义功能区选项卡
        private const string TabId_RESD = "MSDI_RESD";
        private const string TabName_RESD = "MSDI_RESD";
        private const string TabTitle_RESD = "路基工程量";
        ...
        [CommandMethod(AddinOptions.GroupCommnad, CmdStartRibbon, SQConstants.ModelState)]
        public void CreateRibbon()
        {
            ...
            // 如果已经加载，则显示出来即可
            var existingRibbonTab = ribCntrl.Tabs.FirstOrDefault(r => r.Id == TabId_RESD
                                                                      && r.Title == TabTitle_RESD
                                                                      && r.Name == TabName_RESD);
        ...
        private static void AddControls(RibbonTab ribTab)
        {
            ...
            AddButton(pnl_Project, method: typeof(ProjectInfos).GetMethod(ProjectInfos.CommandName, new Type[0]),
                size: RibbonItemSize.Large);

            // ----------------------------- 路基横断面系统 ----------------------------------------
            var pnl_SubgSystem = CreatePanel(ribTab, "路基系统");
            AddButton(pnl_SubgSystem,
                method: typeof(SectionsConstructor).GetMethod(SectionsConstructor.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(pnl_SubgSystem,
                method: typeof(StationNavigator).GetMethod(StationNavigator.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(pnl_SubgSystem,
                method: typeof(LongitudinalSectionDrawer).GetMethod(LongitudinalSectionDrawer.CommandName, new Type[0]),
                size: RibbonItemSize.Large);

            var spltBtn3 = CreateSplitButton(pnl_SubgSystem, "横断面信息");
            AddButton(spltBtn3, method: typeof(SectionWalker).GetMethod(SectionWalker.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(spltBtn3,
                method: typeof(SectionInfosPlayer).GetMethod(SectionInfosPlayer.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            ...
            AddButton(spltBtn1, method: typeof(SlopeCutter).GetMethod(SlopeCutter.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(spltBtn1, method: typeof(SlopeSegMerge).GetMethod(SlopeSegMerge.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            //
            AddButton(pnl_Slope, method: typeof(ProtectionPlacer).GetMethod(ProtectionPlacer.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(pnl_Slope,
                method: typeof(SlopeProtTextModifier).GetMethod(SlopeProtTextModifier.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(pnl_Slope,
                method: typeof(ProtectionFlusher).GetMethod(ProtectionFlusher.CommandName, new Type[0]),
                size: RibbonItemSize.Large);

            // ----------------------------- 工程量的提取 ----------------------------------------
            var pnl_Quantity = CreatePanel(ribTab, "工程量提取");
            AddButton(pnl_Quantity, method: typeof(SubgradeEnvir).GetMethod(SubgradeEnvir.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(pnl_Quantity, method: typeof(CriterionEditor).GetMethod(CriterionEditor.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(pnl_Quantity,
                method: typeof(InfosGetter_Slope).GetMethod(InfosGetter_Slope.CommandName, new Type[0]),
                size: RibbonItemSize.Large);

            var spltBtn2 = CreateSplitButton(pnl_Quantity, "一般工程量");
            AddButton(spltBtn2,
                method: typeof(InfosGetter_ThinFill).GetMethod(InfosGetter_ThinFill.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(spltBtn2,
                method: typeof(InfosGetter_HighFill).GetMethod(InfosGetter_HighFill.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(spltBtn2,
                method: typeof(InfosGetter_SteepSlope).GetMethod(InfosGetter_SteepSlope.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(spltBtn2,
                method: typeof(InfosGetter_StairExcav).GetMethod(InfosGetter_StairExcav.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(spltBtn2,
                method: typeof(InfosGetter_RoadSurface).GetMethod(InfosGetter_RoadSurface.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
            AddButton(spltBtn2,
                method: typeof(InfosGetter_Substitution).GetMethod(InfosGetter_Substitution.CommandName, new Type[0]),
                size: RibbonItemSize.Large);

            AddButton(pnl_Quantity,
                method: typeof(InfosGetter_FillCutInters).GetMethod(InfosGetter_FillCutInters.CommandName, new Type[0]),
                size: RibbonItemSize.Large);
