    ...
    public class Exporter_RoadSurface : DataExporter
    {
        ...
        private class RoadSurface : HalfValue
        {
            ...
            public override bool IsMergeable(IMergeable next)
            {
                ...
                if (RoadSurfaceType != front.RoadSurfaceType) return false;
            ...
            public string GetRoadSurfaceDescription()
            {
                switch (RoadSurfaceType)
                {
                    case RoadSurfaceType.填方土质: return "填方土质路面";
                    case RoadSurfaceType.填方岩质: return "填方岩质路面";
                    case RoadSurfaceType.挖方土质: return "挖方土质路面";
                    case RoadSurfaceType.挖方岩质: return "挖方岩质路面";
                    default:
                        return "未知路面类型";
                }
            }
        ...
        /// <summary> 路面类型 </summary>
        private enum RoadSurfaceType
        {
            填方土质,
            填方岩质,
            挖方土质,
        ...
        public void ExportRoadSurface()
        {
            ...
            {
                var roadSurfaceType = GetRoadSurfaceType(sec);
                if (true)
                {
                    var hfdc = _sortedRanges_RoadSurface[sec.XData.Station];
                    hfdc.BackValue.RoadSurfaceType = roadSurfaceType;
                    //
                    hfdc.FrontValue.RoadSurfaceType = roadSurfaceType;
                    //
            ...
            {"起始桩号", "结束桩号", "区间长度", "结构类型（从工程量偏大的角度考虑，左右侧有一侧为填方，则认为此路面为填方，左右侧有一侧为土质，则认为此路面为土质。）"};
            rows.Add(header);
            ...
            for (int i = 0; i < fillSurfaces.Count; i++)
            {
                ...
                rows.Add(new object[]
                {
                    ...
                    // 路面类型
                    rg.BackValue.GetRoadSurfaceDescription(),
                });
            }
        ...
        /// <summary> 判断填方或土质路堑路段 </summary>
        /// <param name="sec"> 道路中心填方高度</param>
        /// <returns></returns>
        private bool IsFillOrCutsoil(SubgradeSection sec)
        {
            var secData = sec.XData;
