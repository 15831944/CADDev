...
namespace RESD.DataExport
{
    ...
    public class Exporter_ThinFillShallowCut : DataExporter
    {
        ...
        {
            /// <summary> 此段区间内的平均处理宽度 </summary>
            public double AverageWidth { get; set; }

            /// <summary> 此段区间内的平均处理高度 </summary>
        ...
        /// <param name="treatedDepth"> 自然地面以下地基加固的高度</param>
        /// <returns></returns>
        public static bool IsThinFill(Database db, SubgradeSection centerAxis, out double width, out double treatedDepth)
        {
                 ...
                 (center.RightSlopeFill == null || center.RightSlopeFill.Value)))
            {
                ...
                {
                    // 低填路堤的自然地面下部地基处理的高度
                    treatedDepth = _criterion.低填处理高度 - (center.CenterElevation_Road - center.CenterElevation_Ground);
                    if (treatedDepth < 0)
                    {
                        treatedDepth = 0;
                    }
        ...
        /// <param name="ground">道路中心线对应的自然地面点</param>
