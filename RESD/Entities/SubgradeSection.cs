...
namespace RESD.Entities
{
    ...
    public class SubgradeSection
    {
        ...
        /// <returns></returns>
        public static SubgradeSection Create(DocumentModifier docMdf, Line centerLine, out string errorMsg)
        {
            errorMsg = null;
            //
        ...
        ///  !!! 对横断面中的关键对象进行搜索与计算 
        /// </summary>
        /// <param name="errorMsg">如果计算出错，给出出错信息</param>
        /// <returns></returns>
        public bool CalculateSectionInfoToXData(string errorMsg)
        {
        ...
        private static BlockReference FindInfoBlock(Editor ed, Line l)
        {
            ...
            var pt1 = new Point3d(bottomP.X - SQConstants.AxisBottomXOffset, bottomP.Y - SQConstants.AxisBottomYOffset, bottomP.Z);
            var pt2 = new Point3d(bottomP.X + SQConstants.AxisBottomXOffset, bottomP.Y, bottomP.Z);
            ...
            if (res.Status == PromptStatus.OK)
            {
                ...
                // 可能找到多条块参照，比较块参照中点到道路中心线两个端点距离最小的那个块参照
