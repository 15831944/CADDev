...
using eZcadTools.Addins.Geometry;
...
namespace eZcadTools.Addins.Geometry
{
    /// <summary> 对多段线的疏密进行重新设置 </summary>
    [EcDescription(CommandDescription)]
    internal class PolylineRemesh : ICADExCommand
    {
            ...
            ref IList<ObjectId> elementSet)
        {
            ...
            return AddinManagerDebuger.DebugInAddinManager(s.RemeshPolyline,
        ...
        private ExternalCmdResult RemeshPolyline(DocumentModifier docMdf, SelectionSet impliedSelection)
        {
            ...
            if (byVertice)
            {
                ...
                if (seg > 0)
                {
                    ...
                    foreach (var pl in pls)
                    {
                        ...
                        var newCs = eZcad.Utility.Utils.GetThinedPolyline(cs.GetCurves(), seg, aa);
            ...
            else
            {
                ...
                if (segLength > 0)
                {
                    ...
                    {
                        var cs = pl.GetGeCurve() as CompositeCurve3d;
                        var newCs = eZcad.Utility.Utils.GetThinedPolyline(cs, segLength);
                        if (newCs != null)
                        {
                            changedPolylines.Add(pl, newCs);
                        }
