...
namespace eZcadTools.Addins
{
    ...
    public class BlockRefEditor : ICADExCommand
    {
        ...
        /// <summary> 将多个块的属性值进行统一编辑 </summary>
        [CommandMethod(eZcadTools.AppSetup.AddinOptions.GroupCommnad, CommandName,
            CommandFlags.Interruptible | CommandFlags.UsePickSet | CommandFlags.NoBlockEditor)
        , DisplayName(CommandText), Description(CommandDescription)
        , RibbonItem(CommandText, CommandDescription, eZcadTools.AppSetup.AddinOptions.CmdImageDirectory + "EditBlockRefText_32.png")]
        ...
        private Dictionary<BlockReference, string> GetBlockRefs()
        {
            ...
            else
            {
                ...
                bool getBlockNameFromUI = true;
                if (getBlockNameFromUI)
                {
                    var blockRef = PickBlockReference();
        ...
        /// <summary> 从所有布局中搜索指定名称的块参照对象 </summary>
        ...
        /// <summary> 从界面中选择一个块参照 </summary>
        public static BlockReference PickBlockReference()
        {
            // 创建一个 TypedValue 数组，用于定义过滤条件
            var filterTypes = new TypedValue[]
            {
            ...
            op.SetRejectMessage("请选择一个块参照对象");
            op.AddAllowedClass(typeof(BlockReference), true);

            //获取当前文档编辑器
            Editor acDocEd = Application.DocumentManager.MdiActiveDocument.Editor;

            // 请求在图形区域选择对象
            var res = acDocEd.GetEntity(op);

            // 如果提示状态OK，表示对象已选
            if (res.Status == PromptStatus.OK)
            {
                var obj = res.ObjectId.GetObject(OpenMode.ForRead) as BlockReference;
                if (obj != null)
                {
                    return obj;
                }
